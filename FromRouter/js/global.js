...
if(isOptusUI==1)
show_waitingpane(top.G_option.wait);
else
...
if(current_url=="software_update.htm"){
ReloadMe();}}}
else{
setIdVal("MSG",my_msg);}
if(top.G_err<0){
...
top.G_timer=0;
return;}},500);}
function show_waitingpane(waittime)
{
ShowProgress(waittime);
$("#waitingpane").show();
setTimeout(function(){
$("#waitingpane").hide();
do_goNext();},waittime*1000);}
...
document.write('<script type="text/javascript" charset="'+pg_charset+'" src="'+jspg+'?ver=1740113860"></script>');}}}}
if(G_top!=-2){
CGIs.unshift("init");
CGIs.unshift("common");
document.write('<script type="text/javascript" src="js/subformvar.js?ver=1740113860"></script>');
document.write('<script type="text/javascript" src="js/maxnum_define.js?ver=1740113860"></script>');}
document.write('<script type="text/javascript" src="js/menu_optus.js"></script>');
var pre_init_ok=0;
function pre_init()
{
if(pre_init_ok){
...
init();
show_signal();
replace_wifi_tabname();
if(current_url!="login.htm"&&current_url!="logout.htm"){
resetTimeout();}},10);});
isOptusUI=1;
var current_url=location.pathname.substring(location.pathname.lastIndexOf('/')+1);
if(current_url!="login.htm"&&current_url!="logout.htm"){
...
do_jload([["cgi/cgi_common.js"]],function(){});}
for(var i=0;i<document.forms.length;i++){
if(document.forms[i].target=="OUTfrm")
document.forms[i].onsubmit=function(){return false;};}}
...
function switchToggle(id,value){
if(value=="on"||value=="1"||value==1||value==true){
$("#"+id).parent("label").addClass("switchOn");
$("#"+id).prop("checked",true);}
else{
$("#"+id).parent("label").removeClass("switchOn");
...
var show_min=(min>1)?" mins":" min";
var show_hour=(hour>1)?" hours":" hour";
time=hour+show_hour+" "+min+show_min+" "+second+show_second;
if(hour>24){
hour=parseInt(parseInt(second_time/60)/60)%24;
var day=parseInt(parseInt(parseInt(second_time/60)/60)/24);
var show_hour=(hour>1)?" hours":" hour";
var show_day=(day>1)?" days":" day";
time=day+show_day+" "+hour+show_hour+" "+min+show_min+" "+second+show_second;}}}
return time;}
function timeFormater(seconds){
seconds=parseInt(seconds);
var timeString="";
var tempDay=Math.floor(seconds/(86400));
seconds=seconds-(tempDay*86400);
var tempHour=Math.floor(seconds/3600);
seconds=seconds-((tempHour*3600));
var tempMin=Math.floor(seconds/60);
seconds=seconds-((tempMin*60));
timeString=tempDay+"days "+tempHour+"hours "+tempMin+"mins "+seconds+'secs ';
return timeString;}
